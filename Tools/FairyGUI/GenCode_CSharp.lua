...
local function genCode(handler)
    ...
    local memberPrefix = settings.memberNamePrefix or ''

    local classCnt = classes.Count
    local writer = CodeWriter.new({fileMark = '// This is automatically generated by FairyGUI. Please do not modify it.'})
    for i=0,classCnt-1 do
        ...
        for j=0,memberCnt-1 do
            ...
            local propName = upperFirst(memberInfo.name)
            local fieldName = memberPrefix .. propName
            writer:writeln('private %s %s;', memberInfo.type, fieldName)
            writer:writeln('public %s %s => this.%s;', memberInfo.type, propName, fieldName)
        end
        ...
        for j=0,memberCnt-1 do
            ...
            if memberInfo.group==0 then
                if getMemberByName then
                    writer:writeln('%s = (%s)GetChild("%s");', fieldName, memberInfo.type, memberInfo.name)
                else
                    writer:writeln('%s = (%s)GetChildAt(%s);', fieldName, memberInfo.type, memberInfo.index)
                end
            elseif memberInfo.group==1 then
                if getMemberByName then
                    writer:writeln('%s = GetController("%s");', fieldName, memberInfo.name)
                else
                    writer:writeln('%s = GetControllerAt(%s);', fieldName, memberInfo.index)
                end
            else
                if getMemberByName then
                    writer:writeln('%s = GetTransition("%s");', fieldName, memberInfo.name)
                else
                    writer:writeln('%s = GetTransitionAt(%s);', fieldName, memberInfo.index)
                end
            end
        end
        writer:endBlock()
